...
7. **Save** and **Load** in the top right write or read a JSON file on your computer. The fullscreen button fills the window. **Build** unlocks extra rooms (10,000g, then ×3, up to 5: click a pad, then Confirm), extra **tables** and **mannequins** (500g, then ×3 per type; starting pieces do not count), extra **shelves** (same 500g then ×3; the starting three are included), a free **furnace** and **cooking range**, a **spinning wheel** for 500 gp, and a **cauldron** for 10,000 gp. Side rooms clear the exterior trees, grass, beds, and rocks on that footprint so they do not clip through the new room. Garden trees stay off flower beds. The music-note button (right of Load) plays the included track list (Adventure through Yesteryear) and lets you upload more MP3, WAV, or OGG files.
...
Travelers keep a per-class silhouette: Guards in plate with helm, scimitar, and kite; Rangers in hide or a hood with a quiver; Wizards in robes with a hat or hood and a staff orb; Adventurers as civilians (tunic traveler, townsfolk, skirt, cook, rustic worker). Colour variants are original kit names, not product lines.

You start with 40 gold. Bronze gear, a Staff, Green D'hide, and Bread are unlocked.
...
- **Rooms:** from **Build**. Five pads around the starting shop (three behind, one either side). First room 10,000g, then ×3, up to 5 extra spaces. Side expansions clear the exterior trees, grass, and other garden decor on that footprint so they do not clip through the new room.
- **Build / Spinning Wheel:** 500 gp. Place with the floor snap grid, then Confirm. Click it to spin Flax into Bow String. The wheel mesh turns while a craft is running.
- **Build / Furnace:** free. Place with the floor snap grid, then Confirm. Click it to smelt ores into bars. Right-click to move or rotate. A new shop does not start with one; an old save that already placed a furnace keeps it.
- **Build / Cooking Range:** free. Place with the floor snap grid, then Confirm. Click it to bake food. Right-click to move or rotate.
- **Build / Cauldron:** 10,000 gp. Place with the floor snap grid, then Confirm. Click the cauldron to brew potions. Right-click to move or rotate.
...
- **Build / Shelf:** same 500g then ×3 curve. The starting three wall shelves are included; extras place on the wall grid. Right-click to move, rotate, or Display item….
