...
describe('craft preview framing', () => {
  ...
  it('applies a preview-only flip by armour family and leaves the ware mesh alone', () => {
    ...
    assert.equal(craftPreviewEuler('wizard_robe').y, -Math.PI / 2);
    assert.equal(craftPreviewEuler('wizard_robe').x, 0);
    assert.equal(craftPreviewEuler('mystic_robe_top').y, Math.PI / 2);
    assert.equal(craftPreviewEuler('mystic_robe_top').x, 0);
    assert.equal(craftPreviewEuler('splitbark_robe_top').x, Math.PI);
    assert.equal(craftPreviewEuler('bronze_plateskirt').x, Math.PI / 2);
    assert.equal(craftPreviewEuler('dragon_plateskirt').x, Math.PI / 2);
    assert.equal(craftPreviewEuler('green_dragon_mask').z, Math.PI / 2);
    assert.equal(craftPreviewEuler('black_dragon_mask').z, Math.PI / 2);
    assert.equal(craftPreviewEuler('blue_dragon_mask').y, 0);
